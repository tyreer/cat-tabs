...
describe('Tabs Component', () => {
  ...
  describe('Keyboard Navigation', () => {
    ...
    it('navigates to previous tab with left arrow key', async () => {
      ...
      const { rerender } = render(
        <Tabs tabs={mockTabs} activeTabId="tab2" onTabChange={onTabChange} />
      );
      ...
      // Simulate parent updating activeTabId based on onTabChange callback
      expect(onTabChange).toHaveBeenCalledWith(mockTabs[0]);
      rerender(
        <Tabs tabs={mockTabs} activeTabId="tab1" onTabChange={onTabChange} />
      );
    ...
    it('wraps to last tab when pressing left arrow on first tab', async () => {
      ...
      // Simulate parent updating activeTabId based on onTabChange callback
      expect(onTabChange).toHaveBeenCalledWith(mockTabs[2]);
      rerender(
        <Tabs tabs={mockTabs} activeTabId="tab3" onTabChange={onTabChange} />
      );
  ...
  describe('Automatic Activation & Focus Management', () => {
    ...
    it('does not activate already active tab when focused', async () => {
      ...
      // Focus the already active tab directly
      firstTab.focus();
  ...
  describe('Error Handling & Validation', () => {
    it('displays error message for empty tabs array', () => {
      ...
      expect(
        screen.getByText('No tabs provided. Please provide at least one tab.')
      ).toBeInTheDocument();
    });

    it('displays error message for missing id property', () => {
      const invalidTabs = [
        { label: 'Tab 1', content: <div>Content 1</div> } as any,
        { id: 'tab2', label: 'Tab 2', content: <div>Content 2</div> },
      ];
      ...
      expect(screen.getByText('⚠️ Tabs Component Error')).toBeInTheDocument();
      expect(
        screen.getByText("Tab at index 0 is missing required 'id' property.")
      ).toBeInTheDocument();
    });

    it('displays error message for missing label property', () => {
      const invalidTabs = [
        { id: 'tab1', content: <div>Content 1</div> } as any,
        { id: 'tab2', label: 'Tab 2', content: <div>Content 2</div> },
      ];
      ...
      expect(screen.getByText('⚠️ Tabs Component Error')).toBeInTheDocument();
      expect(
        screen.getByText("Tab at index 0 is missing required 'label' property.")
      ).toBeInTheDocument();
    });

    it('displays error message for missing content property', () => {
      const invalidTabs = [
        { id: 'tab1', label: 'Tab 1' } as any,
        { id: 'tab2', label: 'Tab 2', content: <div>Content 2</div> },
      ];
      ...
      expect(
        screen.getByText(
          "Tab at index 0 is missing required 'content' property."
        )
      ).toBeInTheDocument();
    });
    ...
    it('displays error message for duplicate IDs', () => {
      ...
      expect(screen.getByText('⚠️ Tabs Component Error')).toBeInTheDocument();
      expect(
        screen.getByText('All tabs must have unique IDs.')
      ).toBeInTheDocument();
    });

    it('displays multiple error messages for multiple issues', () => {
      const invalidTabs = [
        { label: 'Tab 1' } as any, // missing id and content
        { id: 'tab2', content: <div>Content 2</div> } as any, // missing label
      ];
      ...
      expect(
        screen.getByText("Tab at index 1 is missing required 'label' property.")
      ).toBeInTheDocument();
    });
    ...
    it('validates tabs on prop changes', () => {
      ...
      const invalidTabs = [{ label: 'Tab 1' } as any];
